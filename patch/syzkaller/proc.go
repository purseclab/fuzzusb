...
func (proc *Proc) fuzzUSBInput(item *WorkFzUSB) {
	p := item.p.Clone()
	log.Logf(1, "(fuzzer) #%v: WorkFzUSB executed", proc.pid)
	proc.execute(proc.execOpts, p, ProgNormal, StatFzUSB)
}
