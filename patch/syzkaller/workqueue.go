...
func (wq *WorkQueue) enqueue(item interface{}) {
	...
	case *WorkFzUSB:
		wq.fuzzusb = append(wq.fuzzusb, item)
	default:
...
func (wq *WorkQueue) dequeue() (item interface{}) {
	wq.mu.RLock()
	if len(wq.triageCandidate)+len(wq.candidate)+len(wq.triage)+len(wq.smash)+len(wq.fuzzusb) == 0 {
		wq.mu.RUnlock()
		return nil
	}
	wq.mu.RUnlock()
	...
	if len(wq.fuzzusb) != 0 {
		last := len(wq.fuzzusb) - 1
		item = wq.fuzzusb[last]
		wq.fuzzusb = wq.fuzzusb[:last]
