...
type Fuzzer struct {
	...
	fuzzusbCorpus map[uint64]bool // unique kcov result
}
...
func main() {
	...
	fuzzer := &Fuzzer{
		...
		fuzzusbCorpus: make(map[uint64]bool),
	}
	...
	// 1. get msg from Manager
	//go fuzzer.FzUSBPollLoop()

	// 2. get msg from file within vm
	//go fuzzer.FzUSBPollLoop_file()

	// get kcov result from file, then send them to manager
	go fuzzer.FzUSBLoadKcov()
	go fuzzer.FzUSBSendKcovToMgr()

	// for execute "state_mgr.sh" EDIT
	go fuzzer.stateMgr("hid")

	fuzzer.pollLoop()
}

// for execute "state_mgr.py"
func (fuzzer *Fuzzer) stateMgr(g_name string) {
	cmd := exec.Command("/root/state_mgr.py", g_name) // EDIT
	var out bytes.Buffer
	var stderr bytes.Buffer
	cmd.Stdout = &out
	cmd.Stderr = &stderr
	log.Logf(0, "(fuzzer) before state_mgr.py %v\n", g_name)
	err := cmd.Run()
	if err != nil {
		log.Logf(0, "fail state_mgr.py: %v %v\n", err, stderr.String())
		log.Fatal(err)
	}
	log.Logf(0, "(fuzzer) after state_mgr.py: $v %v\n", g_name, out.String())
}

func (fuzzer *Fuzzer) FzUSBLoadKcov() {
	log.Logf(0, "(fuzzer) FzUSBLoadKcov\n")

	for {
		files, err := ioutil.ReadDir("/root/kcov/")
		if err != nil {
			log.Logf(1, "failed to read dir \"kcov/\": %v\n", err)
			return
		}
		...
		for _, file := range files {
			...
			var f bytes.Buffer
			f.WriteString("/root/kcov/")
			f.WriteString(file.Name())

			readData, err := ioutil.ReadFile(f.String())
			if err != nil {
				log.Logf(1, "failed to read \"%s\": %v\n", f.String(), err)
				continue
			}
			...
			for _, line := range lines {
				...
				// conversion: string -> hexa
				addr, _ := strconv.ParseUint(s, 16, 64)

				FzUSBKcovMu.Lock()
				if fuzzer.fuzzusbCorpus[addr] == false {
					fuzzer.fuzzusbCorpus[addr] = true
					log.Logf(0, "(fuzzer) kcov...unique addr: %v", addr)
				}
				FzUSBKcovMu.Unlock()
			}
			log.Logf(0, "(fuzzer) loaded a kcovfile: %s\n", file.Name())

			// remove the file
			e := os.Remove(f.String())
			if e != nil {
				log.Fatal(e)
			}
		}
...
func (fuzzer *Fuzzer) FzUSBLoadKcov_fsg_thread() {
	log.Logf(0, "(fuzzer) FzUSBLoadKcov_proc\n")

	procCorpus := make(map[string]bool) // checked proc

	for {
		files, err := ioutil.ReadDir("/proc/fzusb/")
		if err != nil {
			log.Logf(1, "(fuzzer) failed to read dir \"/proc/fzusb/\": %v\n", err)
		...
		for _, file := range files {
			...
			// filter checked proc file
			if procCorpus[file.Name()] == true {
				continue
			}
			procCorpus[file.Name()] = true

			var f bytes.Buffer
			f.WriteString("/proc/fzusb/")
			f.WriteString(file.Name())
			...
			for _, line := range lines {
				...
				if fuzzer.fuzzusbCorpus[addr] == false {
					fuzzer.fuzzusbCorpus[addr] = true
					log.Logf(0, "(fuzzer) fzusb kcov...unique addr: %v", addr)
				}
				FzUSBKcovMu.Unlock()
			}
			log.Logf(0, "(fuzzer) loaded a kcovfile: /proc/fzusb/%s\n", file.Name())
		}
	}
}

func (fuzzer *Fuzzer) FzUSBSendKcovToMgr() {
	for {
		time.Sleep(3000 * time.Millisecond)

		// set up data to send
		a := &rpctype.FzUSBKcovArgs{
			Id:   0,
			Name: fuzzer.name,
			Kcov: make(map[uint64]bool),
		}

		// copy current coverage
		FzUSBKcovMu.Lock()
		size := len(fuzzer.fuzzusbCorpus)
		for i, c := range fuzzer.fuzzusbCorpus {
			a.Kcov[i] = c
		}
		FzUSBKcovMu.Unlock()

		log.Logf(0, "(fuzzer) FzUSBSendKcovToMgr: reply #kcov=%v", size)

		// remote call "FzUSBKcov"
		r := &rpctype.FzUSBPollRes{}
		if err := fuzzer.manager.Call("Manager.FzUSBKcov", a, r); err != nil {
			panic(err)
		}
	}
}
