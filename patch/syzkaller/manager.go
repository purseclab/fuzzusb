...
var (
	...
	flagStartCov    = flag.Int("sc", 10, "Starting time for printing out covered input") // EDIT
	flagIntervalCov = flag.Int("ic", 10, "Interval for printing out covered input") // EDIT
)
...
func RunManager(cfg *mgrconfig.Config, target *prog.Target, sysTarget *targets.Target, syscalls []int) {
	...
	mgr := &Manager{
		...
		FzUSBKcov: make(map[uint64]bool),
	}
	...
	mgr.baseAddr, err = getVMOffset(vmlinux, mgr.cfg.TargetOS)
	if err != nil {
		return
	}

	// FzUSB KCOV dump
	mins := time.Duration(*flagStartCov)
	interval := time.Duration(*flagIntervalCov)
	go mgr.FzUSBKcovDump(mins, interval, vmlinux)

	mgr.vmLoop()
}

func (mgr *Manager) FzUSBKcovDump(mins time.Duration, interval time.Duration, vmlinux string) {

	for {
		if (time.Since(mgr.startTime) / 1e9 * 1e9) > mins*60*1000000000 {
			//if (time.Since(mgr.startTime) / 1e9 * 1e9) > mins*1000000000 { // EDIT for state_mgr_10min.py

			FzUSBKcovMu.Lock()
			size := len(mgr.FzUSBKcov)
			covArray := make([]uint64, 0, size)

			for pc := range mgr.FzUSBKcov {
				//pcs = append(pcs, cover.RestorePC(pc, mgr.baseAddr)-callLen)
				//covArray = append(covArray, uint32(pc))
				log.Logf(0, "pcs: {%x \n", pc)
			...
			coveredFrames, err := symb.SymbolizeArray(vmlinux, covArray)
			if err != nil {
				log.Logf(0, "symbolize failure")
				break
			}

			for _, c := range coveredFrames {
				log.Logf(0, "pc: {0x%x %s %s %d %v}\n", c.PC, c.Func, c.File, c.Line, c.Inline)
			}
...
func (mgr *Manager) updateKcov(Kcov map[uint64]bool) {
	FzUSBKcovMu.Lock()
	for i, _ := range Kcov {
		if mgr.FzUSBKcov[i] == false {
			log.Logf(0, "Manager updateKcov added")
			mgr.FzUSBKcov[i] = true
		}
	}
	FzUSBKcovMu.Unlock()
}
