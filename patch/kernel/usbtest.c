...
struct usbtest_param_32 {
	...
	unsigned		is_iso;
	unsigned		is_dir_out;
  struct usb_ctrlrequest ctrl_out;
...
static int
fz_get_endpoints(struct usbtest_dev *dev, struct usb_interface *intf, int fix_alt)
{
	int				tmp;
...
found:
	...
	if (in) {
		...
		dev->info->in_bulk_pipe = dev->in_pipe;
  }
	if (out) {
		dev->out_pipe = usb_sndbulkpipe(udev, out->desc.bEndpointAddress & USB_ENDPOINT_NUMBER_MASK);
		dev->info->out_bulk_pipe = dev->out_pipe;
  }

	if (iso_in) {
		dev->iso_in = &iso_in->desc;
		dev->in_iso_pipe = usb_rcvisocpipe(udev, iso_in->desc.bEndpointAddress & USB_ENDPOINT_NUMBER_MASK);
		dev->info->in_iso_pipe = dev->in_iso_pipe;
		dev->info->iso_in = dev->iso_in;
	}

	if (iso_out) {
		dev->iso_out = &iso_out->desc;
		dev->out_iso_pipe = usb_sndisocpipe(udev, iso_out->desc.bEndpointAddress & USB_ENDPOINT_NUMBER_MASK);
		dev->info->out_iso_pipe = dev->out_iso_pipe;
		dev->info->iso_out = dev->iso_out;
	}

	if (int_in) {
		dev->int_in = &int_in->desc;
		dev->in_int_pipe = usb_rcvintpipe(udev, int_in->desc.bEndpointAddress & USB_ENDPOINT_NUMBER_MASK);
		dev->info->in_int_pipe = dev->in_int_pipe;
		dev->info->int_in = dev->int_in;
	}

	if (int_out) {
		dev->int_out = &int_out->desc;
		dev->out_int_pipe = usb_sndintpipe(udev, int_out->desc.bEndpointAddress & USB_ENDPOINT_NUMBER_MASK);
		dev->info->out_int_pipe = dev->out_int_pipe;
		dev->info->int_out = dev->int_out;
	}
	return 0;
}
...
// populate urb buffer with mutated data
static void fz_simple_fill_buf(struct urb *urb, char *data)
{
	unsigned	i;
...
)
{
	...
	while (retval == 0 && iterations-- > 0) {
		...
		if (usb_pipeout(urb->pipe)) {
		  if (tdev->host_data)
			  fz_simple_fill_buf(urb, tdev->host_data);
      else
			  simple_fill_buf(urb);
...
static void empty_complete(struct urb *urb)
{
	struct ctrl_ctx		*ctx = urb->context;
	struct usb_ctrlrequest	*reqp;
	struct subcase		*subcase;
...
static void
check_active_eps(struct usb_host_interface *interface)
{
	unsigned int n = 0;
	for (n = 0; n < interface->desc.bNumEndpoints; n++) {
		if (usb_endpoint_is_int_in(&interface->endpoint[n].desc))
      printk (KERN_INFO "int-in");
		if (usb_endpoint_is_int_out(&interface->endpoint[n].desc))
      printk (KERN_INFO "int-out");
		if (usb_endpoint_is_bulk_in(&interface->endpoint[n].desc))
      printk (KERN_INFO "bulk-in");
		if (usb_endpoint_is_bulk_out(&interface->endpoint[n].desc))
      printk (KERN_INFO "bulk-out");
		if (usb_endpoint_is_isoc_in(&interface->endpoint[n].desc))
      printk (KERN_INFO "iso-in");
		if (usb_endpoint_is_isoc_out(&interface->endpoint[n].desc))
      printk (KERN_INFO "iso-out");
  }
}

static int
host_fuzz(struct usbtest_dev *dev, struct usbtest_param_32 *param, int pipe, char *desc)
{
	int retval;
	struct urb *urb;
  void *hex = NULL;
  ...
  check_active_eps(interface);

  if (param->data_size) // hexa str -> 1/2 size
	  urb = usbtest_alloc_urb(testdev_to_usbdev(dev),
			pipe, param->data_size/2-1, 0, 1, 0, simple_callback); 
  else {
	  urb = usbtest_alloc_urb(testdev_to_usbdev(dev),
			pipe, param->length, 0, 1, 0, simple_callback);
  }
  ...
  // hexa str -> hex bin
  if (param->data_size) {
	  char *kern_buf = kmalloc(param->data_size + 1, GFP_KERNEL);
	  if (!kern_buf)
		  return -ENOMEM;

	  int status = copy_from_user(kern_buf, param->data, param->data_size);
	  if (status != 0) {
		  return -ENOMEM;
	  }
    kern_buf[param->data_size] = '\0';

	  hex = kmalloc(param->data_size/2, GFP_KERNEL);
	  if (!hex) {
	    kfree(kern_buf);
		  return -ENOMEM;
	  }
    hex2bin(hex, kern_buf, param->data_size/2);
	  kfree(kern_buf);

    // pass to populate with this
    dev->host_data = hex;
  }

	retval = simple_io(dev, urb, param->iterations, 0, 0, desc);
...
static int
host_fuzz_int(struct usbtest_dev *dev, struct usbtest_param_32 *param, int pipe, int interval, char *desc)
{
	int retval;
  ...
  check_active_eps(interface);
	...
	struct urb *urb = usbtest_alloc_urb(testdev_to_usbdev(dev),
			pipe, param->length, URB_NO_TRANSFER_DMA_MAP, 0, interval, simple_callback);
...
static int
host_fuzz_iso(struct usbtest_dev *dev, struct usbtest_param_32 *param, int pipe, struct usb_endpoint_descriptor *iso_ep,	int interval, char *desc)
{
	int retval;
  ...
  check_active_eps(interface);
	...
	retval = test_queue(dev, param, pipe, iso_ep, 1);
	return retval;
}

static int
host_ctr_fuzz(struct usbtest_dev *dev, struct usbtest_param_32 *param)
{
	struct usb_device	*udev = testdev_to_usbdev(dev);
	struct urb		**urb;
	struct ctrl_ctx		context;
	...
	context.dev = dev;
	init_completion(&context.complete);
	context.count = sglen * param->iterations;
	context.pending = 0;
	context.status = -ENOMEM;
	context.param = param;
	context.last = -1;
	...
	for (i = 0; i < sglen; i++) {
		int	pipe = usb_rcvctrlpipe(udev, 0);
		struct urb		*u;
		struct usb_ctrlrequest req = param->ctrl_out;
		struct subcase		*reqp;

		int	expected = 0;

		urb[i] = u = simple_alloc_urb(udev, pipe, le16_to_cpu(req.wLength), 0);
		if (!u)
			goto cleanup;
		...
		if (!reqp)
			goto cleanup;
		reqp->setup = req;
		reqp->number = i % 16; // NUM_SUBCASES
		reqp->expected = expected;
		u->setup_packet = (char *) &reqp->setup;

		u->context = &context;
		u->complete = empty_complete;
	}
...
static void
fz_ioctl (struct usb_interface *intf, struct usbtest_param_32 *param)
{
  ...
  if (param->is_set_alt) { 
    return usb_set_interface(interface_to_usbdev(intf), param->iterations, param->length);
  }
  if (param->is_ctrl) { 
		retval = host_ctr_fuzz(dev, param);
    return retval;
  }

  if (param->is_bulk && param->is_dir_out) {
		retval = host_fuzz(dev, param, dev->info->out_bulk_pipe, "test17");
  }
  if (param->is_bulk && !param->is_dir_out) {
		retval = host_fuzz(dev, param, dev->info->in_bulk_pipe, "test18");
  }
  if (param->is_int && param->is_dir_out) {
		retval = host_fuzz_int(dev, param, dev->info->out_int_pipe, 0, "test25");
  }
  if (param->is_int && !param->is_dir_out) {
		retval = host_fuzz_int(dev, param, dev->info->in_int_pipe, 0, "test26");
  }
  if (param->is_iso && param->is_dir_out) {
		retval = host_fuzz_iso(dev, param, dev->info->out_iso_pipe, dev->info->iso_out, 0, "test15");
  }
  if (param->is_iso && !param->is_dir_out) {
		retval = host_fuzz_iso(dev, param, dev->info->in_iso_pipe, dev->info->iso_in, 0, "test16");
  }
...
usbtest_do_ioctl(struct usb_interface *intf, struct usbtest_param_32 *param)
{
  ...
  // here let's take fz specific control flow
  if (param->is_fz) {
    fz_ioctl(intf, param);
    return 0;
  }
...
usbtest_probe(struct usb_interface *intf, const struct usb_device_id *id)
{
    ...
    int intf_num = intf->altsetting->desc.bInterfaceNumber;

		if (info->need_fix_alt) {
			int status;
      if (info->fix_alt_intf0 > -1 && intf_num == 0) {
			  status = fz_get_endpoints(dev, intf, info->fix_alt_intf0);
			  if (status < 0) {
      ...
      if (info->fix_alt_intf1 > -1 && intf_num == 1) {
			  status = fz_get_endpoints(dev, intf, info->fix_alt_intf1);
			  if (status < 0) {
				  WARNING(dev, "couldn't get endpoints, %d\n", status);
				  kfree(dev->buf);
				  kfree(dev);
      ...
      if (info->fix_alt_intf2 > -1 && intf_num == 2) {
			  status = fz_get_endpoints(dev, intf, info->fix_alt_intf2);
			  if (status < 0) {
				  WARNING(dev, "couldn't get endpoints, %d\n", status);
				  kfree(dev->buf);
				  kfree(dev);
		...
		else if (override_alt >= 0 || info->autoconf) {
			int status;
...
static struct usbtest_info generic_fz_info = {
	.name		= "generic",
	.alt		= -1,
};

static struct usbtest_info ecm_fz_info = {
	.name		= "ecm/ncm/phonet",
	.ep_in		= 2,
	.ep_out		= 2,
	.alt		= 1,
...
static struct usbtest_info uac2_fz_info = {
	.name		= "uac2/uac/",
	.iso		= 1,
	.need_fix_alt = 1,
	.fix_alt_intf0 = -1,
...
static struct usbtest_info ss_fz_info = {
	.name		= "ss/",
	.iso		= 1,
	.need_fix_alt = 1,
	.fix_alt_intf0 = 1,
...
static struct usbtest_info prt_fz_info = {
	.name		= "prt/lb/serial",
	.need_fix_alt = 1,
	.fix_alt_intf0 = 0,
	.fix_alt_intf1 = -1,
...
static struct usbtest_info phonet_fz_info = {
	.name		= "phonet",
	.need_fix_alt = 1,
	.fix_alt_intf0 = -1,
	.fix_alt_intf1 = 1,
...
static struct usbtest_info audiosrc_fz_info = {
	.name		= "audiosrc",
	.iso		= 1,
	.need_fix_alt = 1,
	.fix_alt_intf0 = -1,
...
static struct usbtest_info hid_fz_info = {
	.name		= "hid",
	.need_fix_alt = 1,
	.intr = 1,
	.fix_alt_intf0 = 0,
...
static struct usbtest_info acm_fz_info = {
	.name		= "acm/rndis/midi",
	.need_fix_alt = 1,
	.fix_alt_intf0 = -1,
	.fix_alt_intf1 = 0,
...
static const struct usb_device_id id_table[] = {
	...
	/* generic */
	{ USB_DEVICE(0x0525, 0xa411),
		.driver_info = (unsigned long) &generic_fz_info,
	},

	{ USB_DEVICE(0x0525, 0xa412),
		.driver_info = (unsigned long) &hid_fz_info,
	},

	{ USB_DEVICE(0x0525, 0xa414),
		.driver_info = (unsigned long) &uac2_fz_info,
	},

	{ USB_DEVICE(0x0525, 0xa413),
		.driver_info = (unsigned long) &ecm_fz_info,
	},

	{ USB_DEVICE(0x0525, 0xa415),
		.driver_info = (unsigned long) &ss_fz_info,
	},

	{ USB_DEVICE(0x0525, 0xa416),
		.driver_info = (unsigned long) &prt_fz_info,
	},

	{ USB_DEVICE(0x0525, 0xa417),
		.driver_info = (unsigned long) &phonet_fz_info,
	},

	{ USB_DEVICE(0x0525, 0xa418),
		.driver_info = (unsigned long) &acm_fz_info,
	},

	{ USB_DEVICE(0x0525, 0xa419),
		.driver_info = (unsigned long) &audiosrc_fz_info,
	},
